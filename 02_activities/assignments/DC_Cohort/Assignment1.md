...
One value system that I have been reflecting on after learning about SQL, especially the idea of primary keys, is customer loyalty cards (ex. Optimum, Triangle, Plum, etc) which assign customers a unique number that can be used to track purchases regardless of payment type, physical/online location etc. There is an embedded value system in the assumption that having a comprehensive set of databases - linked by this unique ID number - is beneficial and permissible for large corporations to have in their possession. The decision to start these programs and hold such large amounts of data about customers was made by humans and reflects a certain capitalist logic of data accumulation and ever more precise insights into what customers are buying, where and with whom. Using knowledge from this class, we can better understand how these seemingly innocent processes link into larger databases and data systems that we have little control over once we scan our points cards.

A parameter specific value embedding that I run into in my daily life is that input requirements that allow databases to function do not represent reality. An example of this is the yearly progress reports that we are required to submit as a part of the PhD program. There are questions about year of study and major requirements achieved that have clear connections to one another as outlined in the program expectations. For example, I would select first year, and zero major requirements completed. What these simplifications do not cover is that someone in upper years may have permutations of years completed and program requirements that do not represent their success in the program overall. Year of study is used as a proxy for expected progress, but this value does not represent how well a student is doing. A student may be going faster than expected but not doing well in anything they do and a student that is taking their time may be doing extremely well but, in the database, they appear as “year 6, few requirements met” which would reflect poorly on them. I think this is a good example of how we expect simplifications to reflect the lived realities we experience but oftentimes it is hard to compare rows of a table with one another based simply on their responses to column parameters.
```
